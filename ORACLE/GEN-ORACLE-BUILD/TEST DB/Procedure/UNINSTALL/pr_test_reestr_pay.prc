...
--Подсчитываем количество и сумму
  BEGIN
    ...
    SELECT
      T.PAY_ID,
      T.AMOUNT,
      ((T.AMOUNT*PCK_CONTRACT.GetRateContractTypeDateRate(
        T.CONTRACT_CODE,
        T.TYPE_PAY_TOOLS,
        T.PAY_DATE,
        'REESTR_SWERKA_TEST'
    ...
    WITH T_REESTR AS
      (SELECT
       ...
       (  SELECT
          ...
          (
          SELECT
            T.PARTNER_AC_CODE||';'||T.PAY_ID||';'||TO_CHAR(T.PAY_DATE, 'YYYYMMDDHH24MISS')||'000'||';'||T.PAY_ID_EXT||';'||T.NUMBER_TPP||';'||T.MSISDN||';'||T.OPERATOR_CODE||';'||T.ACCOUNT||';'||TO_CHAR(T.AMOUNT, 'FM999999990.009999')||';'||T.CURRENCY||';'||T.TYPE_PAY_TOOLS||';'||TO_CHAR(((T.AMOUNT*PCK_CONTRACT.GetRateContractTypeDateRate(
                                                                                                                                                                                                                                                                                                                                              T.CONTRACT_CODE,
                                                                                                                                                                                                                                                                                                                                              T.TYPE_PAY_TOOLS,
                                                                                                                                                                                                                                                                                                                                              T.PAY_DATE,
                                                                                                                                                                                                                                                                                                                                              'REESTR_SWERKA_TEST'
                                                                                                                                                                                                                                                                                                                                            ))/100), 'FM999999990.009999') AS PAY
          FROM
            T_TRANSACTION T
          WHERE
            T.STATUS_ID = 111
